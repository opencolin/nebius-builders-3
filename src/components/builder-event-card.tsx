...
export function BuilderEventCard({ event }: { event: BuilderEvent }) {
  ...
  return (
    <Outer
      {...outerProps}
      className="group flex h-full flex-col overflow-hidden rounded-card border border-ink-200 bg-white transition-all hover:border-ink-300 hover:shadow-soft"
    >
      <div className={`relative h-36 bg-gradient-to-br ${cover}`}>
        <div className="absolute left-4 top-4 flex flex-wrap gap-2">
          <StatePill event={event} />
          {event.isOfficial ? (
            <span className="pill bg-white/90 text-navy-700">Nebius official</span>
          ) : null}
        </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <h3 className="mb-2 text-base font-semibold leading-snug text-ink-900 group-hover:text-navy-700">
          {event.title}
        </h3>
        <p className="mb-4 line-clamp-3 text-sm text-ink-600">{event.description}</p>
        ...
        <p className="mt-3 text-xs text-ink-500">
          Hosted by <span className="font-medium text-ink-700">{event.builderName}</span>
        </p>
      </div>
    </Outer>
  );
