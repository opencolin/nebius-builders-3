        ...
        <div className="container-page">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-3">
            <div className="card lg:col-span-2">
              ...
              <p className="mt-3 text-xs text-ink-500 dark:text-ink-400">{total.toLocaleString()} total votes across all projects · refreshed live</p>
            </div>
            <div className="card">
              ...
              <ol className="mt-4 space-y-3 text-sm">
                {[
                  ...
                  <li key={when} className="flex items-center justify-between rounded-lg border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 p-3">
                    <div>
                      <p className="text-xs uppercase tracking-widest text-ink-500 dark:text-ink-400">{when}</p>
              ...
              <ul className="mt-4 space-y-3 text-sm">
                {eventBlasts.map((b) => (
                  <li key={b.id} className="rounded-lg border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 p-3">
                    <p className="text-xs uppercase tracking-widest text-ink-500 dark:text-ink-400">just sent</p>
              ...
              <ul className="mt-4 grid gap-3 sm:grid-cols-3">
                {eventVolunteerOpportunities.map((o) => (
                  <li key={o.title} className="rounded-lg border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 p-4">
                    ...
                    <p className="text-xs text-ink-500 dark:text-ink-400">{o.timeSlot} · {o.location}</p>
                    ...
                    <p className="mt-2 text-xs text-ink-500 dark:text-ink-400">{o.filled} / {o.max}</p>
                  </li>
                ))}
              </ul>
            </div>
