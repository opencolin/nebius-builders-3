        ...
        <section className="section bg-ink-50 dark:bg-ink-800">
          <div className="container-page">
            ...
            <div className="overflow-hidden rounded-card border border-ink-200 bg-white dark:border-ink-700 dark:bg-ink-900">
              <div className="grid grid-cols-[60px_1fr_120px_140px_140px] gap-4 border-b border-ink-200 bg-ink-50 px-5 py-3 text-xs font-semibold uppercase tracking-widest text-ink-500 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-400">
                <div>Rank</div>
                <div>Builder</div>
              ...
              {ranking.map((b, i) => (
                <div
                  key={b.id}
                  className={`grid grid-cols-[60px_1fr_120px_140px_140px] items-center gap-4 border-b border-ink-200 px-5 py-4 last:border-b-0 dark:border-ink-700 ${
                    i === 0
                      ? "bg-lime/15 dark:bg-lime/10"
                      : i % 2 === 0
                      ? "bg-white dark:bg-ink-900"
                      : "bg-ink-50/40 dark:bg-ink-800/40"
                  }`}
                >
                  <div className="text-base font-semibold text-ink-900 dark:text-ink-50">#{i + 1}</div>
                  ...
                  <div className="text-right text-base font-semibold text-navy-700 dark:text-lime">
                    {formatNumber(b.pointsTotal)}
                  </div>
                  <div>
                    <span className="pill-outline text-[10px]">{tierLabel(b.tier)}</span>
                  </div>
                  <div className="text-xs text-ink-500 dark:text-ink-400">{relativeTime(b.lastActiveAt)}</div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-12 rounded-card border border-ink-200 bg-white p-8 text-center dark:border-ink-700 dark:bg-ink-900">
              <h2 className="text-xl font-semibold dark:text-ink-50">Want on this list?</h2>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="card">
      <p className="text-3xl font-bold text-navy-700 dark:text-lime">{value}</p>
