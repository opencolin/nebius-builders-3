      ...
      <main className="bg-ink-50 dark:bg-ink-800">
                ...
                <tbody className="divide-y divide-ink-200">
                  {ourEvents.map((e) => (
                    <tr key={e.id} className="hover:bg-ink-50">
                      <td className="px-4 py-3 font-medium text-ink-900 dark:text-ink-50">{e.title}</td>
                      <td className="px-4 py-3 text-ink-700 dark:text-ink-200">{formatDate(e.startDateTime)}</td>
                      <td className="px-4 py-3 text-ink-700 dark:text-ink-200">{e.city}</td>
                      <td className="px-4 py-3 text-ink-700 dark:text-ink-200">{e.registered}/{e.capacity}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link href={`/events/${e.slug}/manage`} className="text-sm font-medium text-navy-700 dark:text-lime hover:underline">Manage →</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <section id="devrel" className="section bg-white dark:bg-ink-900">
          <div className="container-page">
            ...
            <p className="mt-2 max-w-2xl text-ink-600 dark:text-ink-300">Everything we surface so your DevRel team can act on what shipped at your event.</p>
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              {[
                ...
                <div key={f.t} className="card">
                  ...
                  <p className="mt-2 text-sm text-ink-600 dark:text-ink-300">{f.b}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="mt-6 grid gap-4 lg:grid-cols-3">
              <div className="card lg:col-span-2">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs uppercase tracking-widest text-ink-500 dark:text-ink-400">Current plan</p>
                    <p className="mt-1 text-2xl font-bold">Pro · $2,000/mo</p>
                    <p className="text-sm text-ink-500 dark:text-ink-400">15 events / month · 1,000 IDE minutes</p>
                  </div>
                  <Link href="/pricing" className="btn-outline">Compare plans</Link>
                </div>
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                  <div>
                    <p className="text-xs text-ink-500 dark:text-ink-400">Events used this cycle</p>
                    ...
                    <p className="mt-1 text-xs text-ink-500 dark:text-ink-400">4 of 15</p>
                  </div>
                  <div>
                    <p className="text-xs text-ink-500 dark:text-ink-400">IDE workspace minutes</p>
                    ...
                    <p className="mt-1 text-xs text-ink-500 dark:text-ink-400">812 of 1,000</p>
                  </div>
                </div>
              </div>
